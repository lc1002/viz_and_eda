...
weather_df %>% 
  group_by(month) %>% 
  summarize(n_obs = n()) ## count the number of observations by month
```
...
``` r
weather_df %>% 
  count(name)  ## slight unorganize dataframe in the end
```
...
It’s nice that summarize produces a dataframe `table` doesn’t, and
that’s bad..
...
``` r
weather_df %>% 
  janitor::tabyl(month,name) ## pivot wider of the group by version above. 
```
...
weather_df %>% 
  ...
  summarize(
    n_obs = n(),
    mean_tmax = mean(tmax, na.rm = TRUE), ## OR Use drop_na command.
